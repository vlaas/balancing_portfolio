...
Repo: `vlaas/balancing_portfolio` · baseline commit: `8b7303f` ("total-return merge", 365 tests green) · status: implemented (§12 errata)
...
- [x] `make_net_tr.py` per §3: classification constants, §2.2 construction, README
      template, `--force` guard, deterministic output
- [x] `tests/data/2026-08-20-net15/` committed, generated by the committed script
      from the committed parent (N5 is the proof)
- [x] `sweep.py` provenance per §6
- [x] `tests/test_net_tr.py` N1–N8; `tests/test_indicators.py` net-root exclusion;
...
- [x] Docs: STRATEGY_DEVELOPMENT (net convention, dataset roles); ARCHITECTURE (the
      ...
      regression
- [x] §9 rerun committed: `results/sweep_vt_net_cbase/*`
...
## 12. Errata (implementation)

1. **§3 README vs N5.** The README names the parent by directory basename
   (`2026-08-20`) and titles the snapshot from parent + rate, never from the CLI
   paths — N5 regenerates into a temp directory with an absolute `SRC_DIR`, and
   any embedded path would break byte-equality with the committed snapshot.
2. **§2.1 check order.** The negative-error band `Δ < −TAU` overlaps the FLAT
   band on `[−FLAT_MAX, −TAU)`; the generator checks the error first, so a
   negative step can never be silently classified flat.
3. **§9 measured.** Confirmation gate passed: top-15 overlap 14/15 (a single
   rank-15/16 swap at the plateau edge), gate advantage +0.0766 net vs +0.0778
   gross full-window Calmar over the plain 50/50, 96/96 grid strategies over the
   gated 50/50, identical robust top
   (`VT TQQQ/BTAL t30 w0-60 QQQ:VOL_EWMA80 gate QQQ<SMA200`). Generator output
   matched §1 exactly: 274 jumps, net yields 0.26/0.91/0.53/1.52/4.92/3.70 %/yr;
   §7 goldens reproduced to the cent on the first run.
