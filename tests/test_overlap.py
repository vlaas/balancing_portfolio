...
from overlap_report import (
    CARRIED, DECISION, PAIRS, PERIODS, haircuts, joint, period_ends, regress, verdict,
)
...
def test_the_registered_pairs_and_carried_slots_are_frozen():
    ...
    assert PERIODS == {"monthly": 12, "weekly": 52}
...
def stats(beta=1.0, r2=0.99, alpha=0.0, resid=0.5, corr=0.99):
    return {"beta": beta, "r2": r2, "alpha_yr": alpha, "resid_yr": resid, "corr": corr}


@pytest.mark.parametrize("pair_id,monthly,weekly,expected", [
    ("P1", stats(alpha=-1.0), stats(), "PASS"),
    ("P1", stats(alpha=0.5), stats(), "PASS"),
    ("P1", stats(alpha=0.51), stats(), "FAIL"),
    ("P1", stats(alpha=-2.0), stats(), "CONDITIONAL"),
    ("P1", stats(alpha=-2.9), stats(), "CONDITIONAL"),
    ("P1", stats(alpha=-3.0), stats(), "FAIL"),
    ("P1", stats(beta=0.96, alpha=-1.0), stats(), "FAIL"),
    ("P1", stats(r2=0.979, alpha=-1.0), stats(), "FAIL"),
    ("P2", stats(alpha=-5.25), stats(), "FAIL"),
    ("P2", stats(r2=0.98, alpha=-2.5), stats(), "CONDITIONAL"),
    ("P3", stats(beta=0.4, r2=0.5, alpha=0.30, resid=0.75), stats(), "PASS"),
    ("P3", stats(alpha=-0.31), stats(), "FAIL"),
    ("P3", stats(alpha=0.1, resid=0.76), stats(), "FAIL"),
    ("P4", stats(alpha=-0.60), stats(), "PASS"),
    ("P4", stats(alpha=0.10), stats(), "PASS"),
    ("P4", stats(alpha=-0.61), stats(), "FAIL"),
    ("P4", stats(r2=0.989), stats(), "FAIL"),
    ("P5", stats(beta=1.03), stats(), "PASS"),
    ("P5", stats(beta=1.031), stats(), "FAIL"),
    # P6 decides on the weekly supplement; the monthly row is ignored.
    ("P6", stats(beta=5.0, r2=0.0), stats(corr=0.90, beta=0.8, alpha=1.5), "PROVISIONAL PASS"),
    ("P6", stats(), stats(corr=0.89), "FAIL"),
    ("P6", stats(), stats(beta=1.21), "FAIL"),
    ("P6", stats(), stats(alpha=-1.51), "FAIL"),
    ("P7", stats(beta=1.7, r2=0.8), stats(), "characterization"),
    ("P8", stats(beta=0.2, r2=0.0), stats(), "documentation"),
])
def test_verdict_grammar(pair_id, monthly, weekly, expected):
    assert verdict(pair_id, monthly, weekly) == expected


def row(pair_id, verdict, monthly_alpha, weekly_alpha=0.0):
    return {"id": pair_id, "verdict": verdict,
            "monthly": {"alpha_yr": monthly_alpha}, "weekly": {"alpha_yr": weekly_alpha}}


def test_haircuts_pin_minus_alpha_floored_at_zero_for_carried_passing_slots_only():
    rows = [
        row("P1", "PASS", -0.6),
        ...
        row("P6", "PROVISIONAL PASS", -9.0, weekly_alpha=-1.0),  # decides weekly
    ]
...
def test_a_run_writes_the_three_artefacts(twin_root, tmp_path):
    ...
    for pair in payload["pairs"][:5]:
        ...
        assert pair["monthly"]["beta"] == pytest.approx(1.0)
        assert pair["monthly"]["r2"] == pytest.approx(1.0)
        assert pair["monthly"]["alpha_yr"] == pytest.approx(0.0, abs=1e-6)
    assert payload["pairs"][5]["verdict"] == "PROVISIONAL PASS"
    assert payload["pairs"][5]["weekly"]["n"] > payload["pairs"][5]["monthly"]["n"]
    ...
    assert md.count("| P1 |") == 2 and md.count("| P8 |") == 2
