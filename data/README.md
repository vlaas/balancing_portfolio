...
| EURUSD | euro spot | `FX_IDC:EURUSD` | — | — | FX single | 1971-01-03 | — | — | — |
| GBPUSD | sterling spot | `FX_IDC:GBPUSD` | — | — | FX single | 1971-01-03 | — | — | — |

**FX bar stamps.** TradingView labels an `FX_IDC` daily bar by its 17:00 New
York *open* — the files carry Sunday-to-Thursday labels and no Fridays, and
the bar labelled 2016-06-23 holds the Brexit-vote crash of Friday the 24th
(open 1.5006, close 1.3675). The bar that closes at 17:00 New York on date D
is therefore labelled D − 1, and `make_usd.py` joins the latest FX bar
labelled **strictly before** the symbol's date: ~5.5 h after the London close
(~6 h after Xetra's), the same-day offset the spec accepts. A same-date join
would take the bar closing on D + 1 — a day of look-ahead.
